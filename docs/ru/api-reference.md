...
- `render` — режим рендеринга: `None` / `"auto"` (по умолчанию, HTTP с автоматическим фолбеком в браузер для SPA), `"http"` (строго HTTP, без браузерного фолбека) или `"browser"` (принудительный рендеринг в браузере, 0.5 мин за запрос)
...
# Или освободить чужую/потерянную сессию по ID
baas.release("d3f1c2b0-...")
```

#### `list_sessions()`

Возвращает живые сессии вашего аккаунта (статусы `ACTIVE` и `IDLE`) —
серверное состояние, а не состояние этого клиента. Сюда попадают браузеры,
запущенные другим процессом или предыдущим запуском вашего скрипта.

Основной сценарий — восстановление после падения: процесс упал, `session_id`
потерян, браузер продолжает висеть и тратить минуты. Находим и закрываем:
...
Каждая сессия — словарь с полями `id`, `browserInstanceId`, `status`,
`startedAt`, `lastActivityAt`, `totalMinutes` и др.
